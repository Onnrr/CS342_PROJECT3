...
int AVOID = 1;
int exist[5] =  {4, 6, 7, 3, 5};  // resources existing in the system
...
void *threadfunc1 (void *a)
{
    ...
    setarray(claim, NUMR, 4, 6, 7, 3, 5);
    rm_claim (claim);
    
    setarray(request1, NUMR, 2, 0, 0, 1, 2);
    pr (tid, "REQ", NUMR, request1);
    rm_request (request1);

    sleep(4);

    setarray(request2, NUMR, 2, 2, 4, 2, 1);
...
void *threadfunc2 (void *a)
{
    ...
    setarray(claim, NUMR, 3, 3, 5, 3, 3);
    rm_claim (claim);

    setarray(request1, NUMR, 1, 1, 1, 1, 1);
    pr (tid, "REQ", NUMR, request1);
    rm_request (request1);

    sleep(2);
    
    setarray(request2, NUMR, 1, 0, 0, 2, 0);
    pr (tid, "REQ", NUMR, request2);
    rm_request (request2);
...
void *threadfunc3 (void *a)
{
    ...
    setarray(claim, NUMR, 4, 4, 4, 3, 4);
    rm_claim (claim);

    setarray(request1, NUMR, 2, 0, 0, 3, 0);
    pr (tid, "REQ", NUMR, request1);
    rm_request (request1);

    sleep(5);
    
    setarray(request2, NUMR, 0, 0, 1, 0, 4);
    pr (tid, "REQ", NUMR, request2);
    rm_request (request2);
